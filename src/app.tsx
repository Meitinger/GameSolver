...
const Tab: React.FC<{
  ...
  const solveGame = async (): Promise<void> => {
    ...
    if (!await new Promise<boolean>(resolve => setRunning(running => { resolve(!running); return true }))) {
      return await alert('Another solver is already running.')
    }
    try {
      setGraph(null)
      await game.load(content)
      const winners = await game.solve(Comlink.proxy(setGraph), interval)
      return await alert(winners.length === 0 ? 'No winning strategy.' : `Player(s) with winning strategy: ${winners.join(', ')}`)
    } catch (error) {
      return await alert(String(error))
    } finally {
      setRunning(false)
    }
  }
  ...
  useEffect(() => {
    if ((game == null) || hoveredNode == null || (arenaCanvas.current == null)) {
      return
    }
  ...
  return (
    <div className='uk-grid-divider uk-grid-small uk-flex-nowrap' data-uk-grid>
      <div className={`uk-width-1-${(graph != null) ? '3' : '1'}`}>
