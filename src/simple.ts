...
export class Simple extends TypedGame<Setup, State> {
  ...
  public getPlayers (): PlayerInfo[] {
    return Object
      .entries(this.getSetup().players)
      .map(([name, player]) => ({ name, color: player.color }))
  }
  ...
  protected getNextStates (state: State): Iterable<[number, Iterable<State>]> {
    return Object
      .values(this.getSetup().players)
      .map((player, index) => [index, player.moves[state] ?? []])
  }
